import httpx, time, re, string, json, os, sys, random, threading, websocket, cursor; cursor.hide()
from names import get_last_name, get_first_name
from capmonster_python import HCaptchaTask
from urllib.request import Request, urlopen
from base64 import b64encode as enc
# from colorama import Fores
...
class Others:
    ...
    def get_usernames() -> str:
        ...
        return f"{config['custom_username']} | {letters}"
...
class Console:
    ...
    def update_title() -> None:
        ...
        while True:
            ...
            os.system(
                f"title Tekky x Discord Gen ^│ Accounts: {accounts_created} ^│ Elapsed: {elapsed_time} ^| v1.1" #Speed: {round(int(accounts_created)/int(end), 1)}/s ^│ 
            )
            # thread_lock.release()
...
class Generator:
    ...
    def __init__(self, invite: str, build_num: int = Others().getClientData(), proxy: str = None) -> None:
        self.invite = invite
    ...
    def __main__(self) -> None:
        ...
        try:
            ...
            if not self.register():
                ...
                Console().sprint(f"{self.token}", True)
                thread_lock.acquire()
                global accounts_created
                accounts_created += 1
                with open("./data/tokens.txt", "a") as tokens:
 ...
 ╚═════╝ ╚══════╝╚═╝╚═╝  ╚═╝╚═╝╚══════╝
    ...
    for _ in range(config["accounts"]):
        try:
            ...
            proxy = {"http://": "http://" + proxi, "https://": "http://" + proxi}
        except:
            proxy = None

        thread = threading.Thread(target=Generator(config["invite"], build_num, proxy).__main__)
